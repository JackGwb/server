...
/**
   ...
   Downgrades the MDL_BACKUP_STMT lock to MDL_BACKUP_ALTER_COPY to allow
...
/**
   ...
   Upgrade the lock of the original ALTER table MDL_BACKUP_STMT
...
bool backup_reset_alter_copy_lock(THD *thd)
{
  ...
  /* Ticket maybe NULL in case of LOCK TABLES */
  if (ticket)
    res= thd->mdl_context.upgrade_shared_lock(ticket, MDL_BACKUP_STMT,
                                              thd->variables.lock_wait_timeout);
  return res;
}
